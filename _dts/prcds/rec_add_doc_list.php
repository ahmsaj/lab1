...
if(isset($_POST['id'] , $_POST['n'],$_POST['d'])){
    ...
    if($n==0){
        echo '<table class="dtsTab holdH" cellspacing="0" cellpadding="0" width="100%" border="0">
        <tr ><th class="cbgw" width="150"></th><th valign="top"><div class="fl" fix="hw:120">';        
        $thisH=$sDay;
        while($thisH<$eDay){
            $thh=($thisH/3600);
            $pbh=$dayPer;
            ...
            echo '<div class="fl" style="width:'.$pbh.'%" >'.clockSty($thh).'</div>';
        }
        echo '</div></th></tr>';
    }        					
    $lastDates=array();
    ...
    $sql="select id,d_start,d_end,doctor,clinic,reserve from dts_x_dates where 
    ((d_start>=$sx_date and d_start<=$ex_date) OR (d_end>=$sx_date and d_end<=$ex_date))  and status not in (5,9) and id!='$id' and ( clinic='$clinic' $q2) and status!=9 order by d_start ASC";//status!=9
    $res=mysql_q($sql);
    $rows=mysql_n($res);
    if($rows>0){
        $i=0;
            ...
            if(noVacation($doctor,$thisDay)){
                ...
                if($s_now < $e+$thisDay  || $s_now<$e2+$thisDay){
                    ...
                    if($sDay<$timePointer){
                        ...
                        $blucks.=drowDatBloc('dblc_s1',$blcWidth);
                    }
                    if($s>0){
                        /*********shift 1*****************************************/					
                                ...
                                if($d['s']>=($s+$thisDay) &&  $d['e']<=($e+$thisDay)){	
                                    if($timePointer!=$short_s){
                                            ...
                                            if($dd['sClin']==0){
                                                ...
                                                //echo '('.date('h:i',$thisDay+$timePointer).'<='.date('h:i',$ee).'&& '.date('h:i',$thisDay+$short_ss).'>='.date('h:i',$ee).')<br>';
                                                    ...
                                                    if($short_ss>$timePointer){
                                                        ...
                                                        e="'.($thisDay+$short_ss).'" 
                                                        title="'.clockStye_per($timePointer/3600).'-'.clockStye_per($short_ss/3600).'">
                                                        </div>';*/
                                                        $blucks.=drowDatBloc($sty,$blcWidth,$thisDay+$timePointer,$thisDay+$short_ss,clockStye_per($timePointer/3600).'-'.clockStye_per($short_ss/3600));
                                                    ...
                                                    if($blcWidth){
                                                        ...
                                                        $blucks.=drowDatBloc($vacaSty,$blcWidth,'','',$vacaTitle);
                                                    }
                                        ...
                                        if($timePointer!=$short_s){
                                            ...
                                            $bs=$thisDay+$timePointer;
                                            $be=$thisDay+$short_s;
                                            $bTitle=clockStye_per($timePointer/3600).'-'.clockStye_per($short_s/3600);
                                            $blucks.=drowDatBloc($sty,$blcWidth,$bs,$be,$bTitle);
                                        }
                                        $timePointer=$short_s;
                                    }
                                    ...
                                    if($blcWidth){
                                        $reserv='e';                                        
                                        if(in_array($d['id'],$reserveArr)){
                                            $reserv=array_search($d['id'],$reserveArr);
                                        }
                                        $blucks.=drowDatBloc('fl dblc_basy Over sDoc'.$sameDoc,$blcWidth,$d['s'],$d['e'],'',$d['id'],$reserv,$timeN);
                                    }
                                    $timePointer=$short_e;
                                }
                            }
                        ...
                        if($timePointer<$e){
                                ...
                                if($dd['sClin']==0){
                                    ...
                                    ($ee >=$thisDay+$timePointer && $ee <=$thisDay+$e)){
                                        ...
                                        if($blcWidth){
                                            ...
                                            $blucks.=drowDatBloc($vacaSty,$blcWidth,'','',$vacaTitle,$action);
                                        }
                                        $timePointer=$short_ee;
                                    }
                                }
                            ...
                            if($timePointer!=$e){
                                ...
                                $bs=$thisDay+$timePointer;
                                $be=$thisDay+$e;
                                $bTitle=clockStye_per($timePointer/3600).'-'.clockStye_per($e/3600);
                                $blucks.=drowDatBloc($sty,$blcWidth,$bs,$be,$bTitle);
                            }
                            $timePointer=$e;
                        }
                    ...
                    if($s2){
                            ...
                            if($d['sClin']){
                                ...
                                if($d['s']>=($s2+$thisDay) &&  $d['e']<=($e2+$thisDay)){	
                                            ...
                                            if($dd['sClin']==0){
                                                ...
                                                ($ee >=$thisDay+$timePointer && $ee <=$thisDay+$short_s)){
                                                    ...
                                                    if($short_ss>$timePointer){
                                                        ...
                                                        $bs=$thisDay+$timePointer;
                                                        $be=$thisDay+$short_ss;
                                                        $bTitle=clockStye_per($timePointer/3600).'-'.clockStye_per($short_ss/3600);
                                                        $blucks.=drowDatBloc($sty,$blcWidth,$bs,$be,$bTitle);
                                                        $timePointer=$short_ss;
                                                    }
                                                    $blcWidth=($short_ee-$short_ss)*100/$dayLength;
                                                    if($blcWidth){
                                                        $blucks.=drowDatBloc('dblc_3',$blcWidth);
                                                    }
                                                    $timePointer=$short_ee;
                                                }
                                            }
                                    ...
                                    if($blcWidth){
                                        ...
                                        $blucks.=drowDatBloc('dblc_basy Over sDoc'.$sameDoc,$blcWidth,$d['s'],$d['e'],'',$d['id'],$reserv,$timeN);
                                    }
                                    $timePointer=$short_e;
                                }
                            }
                        ...
                        if($timePointer<$e2){
                                ...
                                if($dd['sClin']==0){
                                    ...
                                    ($ee >=$thisDay+$timePointer && $ee <$thisDay+$e2)){
                                        ...
                                        if($short_ss>$timePointer){
                                            ...
                                            $bs=$thisDay+$timePointer;
                                            $be=$thisDay+$short_ss;
                                            $bTitle=clockStye_per($timePointer/3600,1).'-'.clockStye_per($short_s/3600);
                                            $blucks.=drowDatBloc($sty,$blcWidth,$bs,$be,$bTitle);
                                            $timePointer=$short_ss;
                                        }
                                        $blcWidth=($short_ee-$short_ss)*100/$dayLength;
                            ...
                            if($timePointer!=$e2){
                                ...
                                $bs=$thisDay+$timePointer;
                                $be=$thisDay+$e2;
                                $bTitle=clockStye_per($timePointer/3600).'-'.clockStye_per($e2/3600);
                                $blucks.=drowDatBloc($sty,$blcWidth,$bs,$be,$bTitle);
                            }
                            $timePointer=$e2;
                        }
                    ...
                    /**************************************************/
                    echo '<td t class="f1" style="background-color:'.$wakeeDaysClr[$thisDayNo].'">
                        <ff14>'.date('m-d',$thisDay).'</ff14> - '.$wakeeDays[$thisDayNo].' 
                    </td>
                    <td c >'.$blucks.'</td>';
                }
                $allDays++;
                ...
                echo '
                <td t class="f1 clrw" style="background-color:'.$wakeeDaysClr[$thisDayNo].'" title="'.$wakeeDays[$thisDayNo].'"><ff>'.date('m-d',$thisDay).'</ff> - '.$wakeeDays[$thisDayNo].'
                </td>
                <td>
                    <div c>
