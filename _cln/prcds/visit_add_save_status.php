...
if(isset($_POST['id'])){
	...
	if($rows>0){
		...
		$sql="select * from  cln_x_visits_services where visit_id='$v_id' order by id ASC";
		$res=mysql_q($sql);
		$rows=mysql_n($res);
		if($rows>0){
			$gm_note=get_val_con('gnr_x_exemption_notes','note'," vis='$v_id' and mood=$cType "); 
            ...
            while($r=mysql_f($res)){					
                $s_id=$r['id'];
                $service=$r['service'];
				list($serviceName,$edit_price,$hPart,$dPart)=get_val('cln_m_services','name_'.$lg.',edit_price,hos_part,doc_part',$service);
				...
				if($pay_type==3){
					...
					$cancelServ='';
					$sur=getRecCon('gnr_x_insurance_rec'," service_x='$s_id' and mood=1");
		...
		<div class="form_fot fr"><? 
            ...
            <div class="cb">
				<?
				$netPayment=$total2;
				$payText=k_cus_must_pay;
				$butCol='bu_t4';
				if($dts_id){
					$dtsPay=DTS_PayBalans($dts_id,$v_id,$cType);
					if($dtsPay){?>
						<div class="f1 clr5 fs16 lh40"><?=k_payment_amount?> : <ff> ( <?=number_format($dtsPay)?> )</ff></div><?
						$netPayment-=$dtsPay;
					}
				}
				if($netPayment<0){
					$netPayment=$netPayment*(-1);
					$payText=k_must_returned_cus;
					$butCol='bu_t3';
				}
				if($netPayment==0){					
					$payText=k_stm;					
				}?>
				<div class="bu <?=$butCol?> fl" onclick="printReceipt(<?=$v_id?>,1)"> <?=$payText?> <ff> ( <?=number_format($netPayment)?> )</ff></div>
				<?				
                if($pay_type==0){?>
