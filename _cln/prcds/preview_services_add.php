...
if(isset($_POST['id'])){
	$id=pp($_POST['id']);
	list($pay_type,$vis_status)=get_val('cln_x_visits','pay_type,status',$id);
	...
	<div class="form_header" type="full">
		...
		if($vis_status==1){
			...
			if($rows>0){
				...
				?>
				<form name="n_visit" id="n_visit" action="<?=$f_path?>X/cln_preview_services_save.php" method="post"  cb="rev_ref(0,<?=$id?>);" bv="id">
				...
				while($r=mysql_f($res)){
					...
					$muliTxt='-';if($multi){$muliTxt='<input type="number" name="m_'.$s_id.'" value="1" min="1" max="3"/>';}
