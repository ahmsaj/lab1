			...
			if($c_type==2){
				...
				if($rows>0){					
					...
					if(getTotalCO('lab_x_visits_services'," visit_id ='$vis' and status=8")){
						echo '<div class="ic40 icc4 iconDelv fr" onclick="lRepDlv(2,'.$vis.',0)" title="'.k_sub_all_results.'"></div>';
					}
					echo '
				...
				if($rows>0){
					...
					<div class="form_body so">
						...
						if($opr_show_service){				
							...
							if($rows_i>0){
								...
								$totalPrice=0;
								$opr_pay_add=1;
								echo '<div class="f1 fs18 clr1 lh40">'.k_adtn_tsts.'</div>';
								echo '<table width="100%" border="0" cellspacing="0" cellpadding="4" class="grad_s" type="static" over="0">';
								...
								while($r=mysql_f($res)){
									...
									if($pay_type==3){$price=$r['pay_net'];}
									$tr_color=$ser_status_color[$s_status];
									$addDes='';														
									//$tr_color='#faa';
									echo '<tr bgcolor="'.$tr_color.'">	
									<td class="ff B fs16">'.$name.$addDes.'</td>
									<td><ff>'.number_format($price).'</ff></td>
									</tr>';			
									$ser_times[$i]['time']=$s_time;
									$ser_times[$i]['name']=$name;
								...
								echo '<tr bgcolor="#f5f5f5">			
									...
									if($pay_type==2 ){
										$chr_amunt=get_val_c('gnr_x_referral_charities','rece_amount',$vis,'vis');
										$totalPrice=$totalPrice-$chr_amunt;
										echo '<tr bgcolor="'.$clr4.'">			
										<td class="f1 fs14 B">'.k_amnt_paid_by_chrty.'</td>
										<td><ff>'.number_format($chr_amunt).'</ff></td>
										</tr>';
									}
									if($visBla>0 ){
										$totalPrice+=$visBla;
										...
										echo '<tr bgcolor="#f5f5f5">			
										<td class="f1 fs14 B">'.k_nt_amnt_req.'</td>
										<td><ff>'.number_format($totalPrice).'</ff></td>
										</tr>';
									}
								echo '</table>';
					...
					<div class="form_fot fr">
						...
						if($opr_new_roles){
							echo '<div class="bu bu_t1 fl" onclick="newRoels('.$vis.','.$c_type.')">'.k_nw_rol.'</div>';}
						if($opr_pay_add){  
							echo '<div class="bu bu_t3 fl w-auto" onclick="addPay('.$vis.','.$totalPrice12.','.$payType.','.$c_type.')">'.$butt.'</div> 
							<input type="number" class="'.$hideInput.'" id="l_pay" max="'.$totalPrice12.'" value="'.$totalPrice12.'" style="width:100px; margin:10px; color:#f00"/>';
						}else{
							//if($EndPay && $Vbal>0){
							if($Vbal>0){
								echo '<div class="bu bu_t3 fl w-auto" onclick="addPay('.$vis.','.$Vbal.',2,'.$c_type.')">'.k_py_rst_due.'</div> 
							<input type="number" disabled class="'.$hideInput.'" id="l_pay" max="'.$Vbal.'" value="'.$Vbal.'" style="width:100px; margin:10px; color:#f00"/>';
							}
						}					
						?>
					</div>
