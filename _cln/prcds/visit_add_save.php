	...
	if($pp&&$cc){
		...
		if($type==3){
			...
			if($vis_id==0){
				...
				if($insert){
					$endoscopic=get_val('gnr_m_clinics','endoscopic',$c);
					$new_pat=isNewPat($p,$doc,$type);
					$sql="INSERT INTO xry_x_visits(`patient`,`clinic`,`d_start`,`reg_user`,`fast`,`emplo`,`doctor`,`doc_ord`,`new_pat`,`endoscopic`)values ('$p','$c','$now','$thisUser','$fast','$emplo','$doc','$doc_ord','$new_pat','$endoscopic')";
					if(mysql_q($sql)){			
						$vis_id=last_id();
					}
				}
					...
					while($r=mysql_f($res)){
						...
						if(isset($_POST['ser_'.$s_id])){
							...
							if($insert2){
								...
								if($multi){$m=pp($_POST['m_'.$s_id]);}
								for($s=0;$s<$m;$s++){							
									mysql_q("INSERT INTO xry_x_visits_services (`visit_id`, `clinic`, `service`, `hos_part`, `doc_part`, `doc_percent`, `pay_net`, `rev`,`d_start`,`total_pay`, `patient`,`doc`,`srv_type`)	values ('$vis_id','$c','$s_id','$hos_part','$doc_part','$doc_percent','$pay_net','$ch_p','$now','$total_pay','$p','$doc','$opr_type')");
									$srv_x_id=last_id();									
									if($doc_ord){
										mysql_q("UPDATE xry_x_visits_requested_items set status=1 , service_id='$srv_x_id'  where r_id='$doc_ord' and action=1 and status=0 and xphoto='$s_id' ");
									}
