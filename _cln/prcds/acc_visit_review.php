...
if(isset($_POST['fil'] , $_POST['p'])){
	...
	$q='';
	$q2='';
	if($thisGrp=='7htoys03le'){$q.=" (doctor = '$thisUser') ";}	
	...
	if($q){$q=" where $q ";}
    $sql="select count(*)c from cln_x_visits  $q ";
	$res=mysql_q($sql);
	$r=mysql_f($res);
	...
	$sql="select * from cln_x_visits $q order by id DESC $q_limit";
	$res=mysql_q($sql);
	$rows=mysql_n($res);	
	if($rows>0){?>
		...
		while($r=mysql_f($res)){
			...
			if($thisGrp=='7htoys03le'){$action='showDocVis('.$id.')';}
			?><tr>
