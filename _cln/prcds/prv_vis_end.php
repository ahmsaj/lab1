...
if(isset($_POST['v_id'])){	
	...
	list($clinic,$dts_id,$patient)=get_val($m_table,'clinic,dts_id,patient',$v_id);
	$q=", work='$work_time' ";
	...
	if($ch1){		
		...
		if($rows>0){
			...
			while($r=mysql_f($res)){	
				...
				if($s_status==0 || $s_status==2){$n_srv=1;}else{$e_srv=1;}
			}				
			if($n_srv==1 && $e_srv==0){// New
                ...
                if($finish){
                    ...
                    if(mysql_q("UPDATE $m_table set status=2 , d_finish='$now' where id='$v_id'")){
                        fixWorkTime($v_id,1);
                        if(_set_ruqswqrrpl){makeSerPayAlert($v_id,$mood,0,'x1');}
                        ...
                        api_notif($patient,$mood,51,$v_id);
                    }
                }
			}else{
				echo '3^';
