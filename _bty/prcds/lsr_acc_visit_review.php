...
if(isset($_POST['fil'] , $_POST['p'])){
	...
	$q='';
	$q2='';
	if(in_array($thisGrp,array('66hd2fomwt'))){$q.=" doctor = '$thisUser'  ";}	
	foreach($pars as $p){
		if($p!=''){
			$pp=explode(':',$p);
			$cal=$pp[0];
	...
	if($q){$q=" where $q ";}
	$sql="select count(*)c from bty_x_laser_visits  $q ";
	$res=mysql_q($sql);
	$r=mysql_f($res);
	...
	$sql="select * from bty_x_laser_visits $q order by id DESC $q_limit";
	$res=mysql_q($sql);
	$rows=mysql_n($res);	
	if($rows>0){?>
        ...
        <th class="fs16 f1"><?=k_specialist?></th>             
		...
		while($r=mysql_f($res)){
			...
			if(in_array($thisGrp,array('66hd2fomwt'))){$action='showDocVis('.$id.')';}
			?><tr>
			...
			<? if($pay_type){ ?><div class="clr5 f1"><?=$pay_types[$pay_type]?></div><? }?></td>
