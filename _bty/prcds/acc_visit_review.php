...
if(isset($_POST['fil'] , $_POST['p'])){
	...
	$q='';
	$q2='';
	if(in_array($thisGrp,array('9yjlzayzp'))){$q.=" doctor = '$thisUser' ";}
	foreach($pars as $p){
		if($p!=''){
			$pp=explode(':',$p);
			$cal=$pp[0];
			...
			if($cal=='p13'){if($q){$q.=" AND ";}$q.=" new_pat = '$val1' ";}
	...
	if($q){$q=" where $q ";}
	$res=mysql_q("select count(*)c from bty_x_visits  $q ");
	$r=mysql_f($res);
	...
	$sql="select * from bty_x_visits $q order by id DESC $q_limit";
	$res=mysql_q($sql);
	$rows=mysql_n($res);	
	if($rows>0){?>
		...
		while($r=mysql_f($res)){
			...
			$action='';
			if($thisGrp=='hrwgtql5wk' || $thisGrp=='o81muv4ggi' || $thisGrp=='s'){$action='showAcc('.$id.',5)';}
			if(in_array($thisGrp,array('9yjlzayzp'))){$action='showDocVis('.$id.')';}
			?><tr>
