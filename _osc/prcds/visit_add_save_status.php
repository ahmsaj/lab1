...
if(isset($_POST['id'])){
	...
	$v_id=pp($_POST['id']);
	$editable=1;
	$sql="select * from osc_x_visits where id='$v_id' ";
	$res=mysql_q($sql);
	$rows=mysql_n($res);
	if($rows>0){
		$r=mysql_f($res);
		...
		$sql="select * from  osc_x_visits_services where visit_id='$v_id' order by id ASC";
		$res=mysql_q($sql);
		$rows=mysql_n($res);
		if($rows>0){
            ...
            while($r=mysql_f($res)){					
				...
				$offer=$r['offer'];
                $doc_fees=$r['doc_fees'];
				list($serviceName,$edit_price,$hPart,$dPart)=get_val('osc_m_services','name_'.$lg.',edit_price,hos_part,doc_part',$service);
				$srvPriceOrg=$hPart+$dPart;
                $hos_part=$r['hos_part'];
                $doc_part=$r['doc_part'];				
				$edit_priceTxt='';
				if($edit_price){$hos_part=$doc_part=0;$edit_priceTxt='<div class="f1 clr6">
				'.k_price_det_by_dr.'</div>';}
                $pay_net=$r['pay_net'];
				$rev=$r['rev'];
				...
				if($pay_type==3){
					...
					$cancelServ='';
					$sur=getRecCon('gnr_x_insurance_rec'," service_x='$s_id' and mood= 7");
		...
		<div class="form_fot fr"><? 
            ...
            <div class="cb">
                ...
                <? 
                if($pay_type==0 && _set_qejqlfmies==0){?>
