...
if(isset($_POST['v'] , $_POST['a'], $_POST['m'])){
	$vis=pp($_POST['v']);
	$amount=pp($_POST['a']);
	$mood=pp($_POST['m']);
	$doc=pp($_POST['doc']);
	...
	if($p){
		...
		if($rows>0){
			if($mood==6){
				...
				$bal=$srvs-$lastPay;
				if($bal){$pay=addPay4($vis,11,$amount,$doc);}
                ...
                $all_add=get_sum($table2,'pay_net',"visit_id='$vis' and status IN(2,5)");
				$all_del=get_sum($table2,'pay_net',"visit_id='$vis' and status IN(3,4)");
				...
				$balans=$all_add-$all_del-$all_insur-$bankPay;                
				$p1=$p2=$p3=1;
				if($balans<0){$balans=$balans*(-1);}                
				if($balans==$amount){
                    if($bankPay){
                        echo addCashPay2($vis,$mood,$balans);
                    }else{
                        if($all_add){
					...
					if($all_insur){
						$sql="select * from gnr_x_insur_pay_back where visit='$vis' and patient='$p' ";
						$res=mysql_q($sql);
						while($r=mysql_f($res)){
							$id=$r['id'];
							$in_vis=$r['visit'];
                            ...
                            if($mood==5){$p3=addPay5($in_vis,6,$amount);}
