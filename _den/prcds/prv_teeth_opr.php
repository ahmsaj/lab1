    ...
    if($r['r']){
        ...
        if($status==1 && $thisUser==$doctor){
            if($oprT==1){//add Status
                mysql_q("UPDATE den_x_opr_teeth SET `last_opr`=0 where `patient`='$patient' and `teeth_no`='$teeth' and teeth_part='$type'");                
                ...
                $sql="delete from den_x_opr_teeth  where patient='$patient' and teeth_no='$teeth' and teeth_part='$type' and opr_type='$opr_type' and opr='$opr' and opr_sub='$oprSub' and `last_opr`=1";
