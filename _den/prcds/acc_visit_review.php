...
if(isset($_POST['fil'] , $_POST['p'])){
	...
	$q='';
	$q2='';
	if($thisGrp=='fk590v9lvl'){$q.=" (doctor = '$thisUser') ";}	
	foreach($pars as $p){
		if($p!=''){
			$pp=explode(':',$p);
			$cal=$pp[0];
	...
	if($rows>0){?>
		...
		while($r=mysql_f($res)){
			...
			$action='';
			if($thisGrp=='hrwgtql5wk'){$action='showAcc('.$id.','.$cType.')';}
			if($thisGrp=='fk590v9lvl' || $thisGrp=='s' ){$action='denHis('.$patient.','.$id.')';}
			?><tr>
			...
			<td class="f1">
				<div class="fr ic40 icc1 ic40_info" onclick="<?=$action?>"></div>
				<div class="fr ic40 icc2 ic40_price" onclick="accStat(<?=$patient?>)"></div>
			</td>
			</tr><?
		}		
	}else{
