...
if(isset($_POST['vis'],$_POST['r'],$_POST['sr'],$_POST['cat'])){
	...
	if($cat==0 && $sr==''){
		...
		$rows=mysql_n($res);
		echo '<div>
			<div class="fl pd10  f1 fs14">'.k_cats.' : <ff>'.$rows.'</ff></div>
		</div>^';		
		if($rows){
			while($r=mysql_f($res)){
				$id=$r['id'];				
				$name=$r['name_'.$lg];								
				echo '<div class="f1  pd10 pd5v br5 mg10v cbgw bord Over2 TC lh30" hisCat="'.$id.'">'.$name.'</div>';
			}
		}
	}else{
		$p=25;
		...
		if($rows){
			while($r=mysql_f($res)){
				...
				$cName='';
				if($cat==0 && $sr){					
					$cName='<div class="f1  pd10 clr6 lh30">'.get_val_arr('cln_m_medical_his_cats','name_'.$lg,$r['cat'],'c').'</div>';
				}
				echo '<div ih="'.$id.'"  >'.$cName.'			
					<div class="mg10 f1 " tit >'.hlight($sr,$name).'</div>
				</div>';
			}
			if($rows<$all){
