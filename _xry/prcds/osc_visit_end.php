...
if(isset($_POST['v_id'],$_POST['t'])){	
	...
	if($status!=''){		
		if($status>1){
			...
			$visData=getTotalCO('xry_x_visits_services_add'," visit_id='$v_id' ");
			if($t==1){
				echo '1^'?>						
				<div class="win_body">		 
					...
					<div class="form_fot fr">
						...
						<? if($visData){
							...
							echo '<div class="bu bu_t3 fl" onclick="cancelXryVisit('.$v_id.')">'.k_cancel_visit.'</div>';
						}?>
					</div>
				</div><?
			}
			if($t==2){
				echo '2^';
				if($status==1){
					if(mysql_q("UPDATE xry_x_visits set status=2 , d_finish='$now' where id='$v_id'")){
                        fixWorkTime($v_id,3);
						//if(_set_ruqswqrrpl){makeSerPayAlert($v_id,$mood);}					
						mysql_q("UPDATE gnr_x_roles set status=4 where vis='$v_id' and mood='$mood' ");
						mysql_q("UPDATE xry_x_visits_services set status=1 where visit_id='$v_id' ");
