	...
	if($r['r']){
		...
		<div class="fr clr5"><ff>#<?=$id?></ff></div>
			...
			<div class="form_body so" type="full"><? 
				if(($thisUser==$doctor || ($ray_tec==$thisUser && $doctor==0))){?>
					<div class="fs16 f1 lh40 clr1"><?=k_services?></div>
					<div><?
						$ser_times=array();
						$totalTime=0;
						$vis_status=get_val('xry_x_visits','status',$id);
						$timeLimit=0;
						if($vis_status==2){
							$vis_finish=get_val('xry_x_visits','d_finish',$id);
							if(($now-$vis_finish)<40000){$timeLimit=1;}
						}

						$sql="select * , x.id as xid from xry_m_services z , xry_x_visits_services x where z.id=x.service and x.visit_id='$id' order by z.ord ASC";
						$res=mysql_q($sql);
						$rows=mysql_n($res);
						if($rows>0){				
