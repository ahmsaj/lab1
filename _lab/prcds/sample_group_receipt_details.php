...
if(isset($_POST['id'])){
	...
	$total_samples='';
	if($id){		
		$sql="select pkg_id , count(pkg_id) c from lab_x_visits_samlpes where status=2 and grp='$id' group by pkg_id order by patient ASC ";
		$res=mysql_q($sql);
		$rows=mysql_n($res);
		if($rows>0){
			$i=0;				
		...
		$rows=mysql_n($res);
		?>
		<div class="mg101  fxg h100" fxg="gtr:auto 1fr">
			<div class=" uLine mg10 lh40 ">
				...
				<? if($rec['status']==1){
					echo '<div class="fr ic40 icc2 ic40_done" onclick="receiptSG('.$id.')" title="'.k_receipt.'" ></div>';
				}?>				
				<div class="lh50 fl w100 f1 fs16"><?=$total_samples?></div>&nbsp;
			</div>
			
			<div fix1="hp:51|wp:22" class="mg10 ofx so " ><?
				if($rows>0){
					$i=0;
					...
					while($r=mysql_f($res)){
						...
						if($p!=$thisP){
							$pT=getTotalCO('lab_x_visits_samlpes',"grp='$id' and visit_id ='$visit_id' and patient='$p'");
		...
		$sql="select * from lab_x_visits_samlpes where status=2 and grp=0 order by patient ASC ";
		$res=mysql_q($sql);
		$rows=mysql_n($res);?>
